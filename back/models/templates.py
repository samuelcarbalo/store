...
class Template:
    ...
    def add_template_squad(tournament_id, squad_id, jugadores):
        for player in jugadores:
            ...
            if  "NOMBRE DE LA IGLESIA (SOLO PARA INVITADOS)" in cleaned_dict:
                player_item["guest_church"] = cleaned_dict["NOMBRE DE LA IGLESIA (SOLO PARA INVITADOS)"]
    ...
    def get_template_squad(tournament_id, squad_id):
        ...
        player_squad =  list(db.player.find({"tournament_id": tournament_id, "squad_id":squad_id}))
        ...
        sort_player_squad = sorted(player_squad, key=lambda item: item['number'])
    ...
    def delete_groups(tournament_id):
        group = db.groups.delete_many({"tournament_id": ObjectId(tournament_id)})
        return {
            "deleted": True
        }
    ...
    def find_groups(tournament_id):
        groups = list(db.groups.find({"tournament_id": ObjectId(tournament_id)}))
        # Convertir ObjectId a string para que sea serializable en 
        for grupo in groups:
            print(grupo)
            grupo['_id'] = str(grupo['_id'])
            grupo['tournament_id'] = str(grupo['tournament_id'])
            for equipo in grupo['squads']:
                equipo['squad_id'] = str(equipo['squad_id'])
                squad_ = db.squads.find_one({"_id": ObjectId(equipo['squad_id'])})
                equipo['pj'] = squad_["pj"]
                equipo['pg'] = squad_["pg"]
                equipo['pp'] = squad_["pp"]
                equipo['pe'] = squad_["pe"]
