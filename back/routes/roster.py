...
@roster_bp.route('/<template_id>/squads/<squad_id>/template/edit', methods=['POST']) # Consider changing to PUT method
@admin_required
def update_template_squad_id(template_id, squad_id):
    ...
    try:        
        ...
        if file and allowed_file(file.filename):
            ...
            print(jugadores)
            squad = Template.add_template_squad(template_id, squad_id, jugadores)
            if not squad:
...
def update_player_edit(template_id, squad_id, user_id):
    try:
        ...
        if email:
            ...
            if validate_ == False:
                return jsonify({"warning": "El correo no es valido"}), 500 # Consider 404 if ID not found
        
        user_squad = Template.update_player_squad(template_id, squad_id, user_id, data)
        if not user_squad:
            return jsonify({"error": "Jugador no encontrado"}), 404
        return user_squad
    except Exception as e:
...
@roster_bp.route('/player/delete/<squad_id>/', methods=['GET']) # Consider changing to DELETE method
@admin_required
def delete_player_id(squad_id):
    try:
        ...
        if not player:
            return jsonify({"error": "Jugador no encontrado"}), 404
        return jsonify({"success": True, "message": "Jugador eliminado"}) # Return success message
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def generate_random_groups(template_id):
    try:
        ...
        torneo = Template.find_Template(template_id)
...
@roster_bp.route('/<template_id>/groups/', methods=['GET']) 
@admin_required
def get_groups(template_id):
    try:
        # Consultar todos los groups para el template_id especificado
        groups = Template.find_groups(template_id)
