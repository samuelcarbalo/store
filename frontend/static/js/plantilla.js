...
async function cargarPlantillaParaEditar(data) {
    ...
    try {
        const response = await fetch(`/api/tournaments/squads/${equipoId}/${squad_name}`);
        ...
        btnVerPlantilla.onclick = () => {
            window.location.href = `/squad/${equipoId}/details`
        };
...
async function subirPlantillaExcel(event, equipoId) {
    ...
    try {
        ...
        const response = await fetch(`/api/tournaments/${torneoId}/squads/${equipoId}/plantilla`, {
            method: 'POST',
            body: formData
...
async function eliminarEquipo(equipoId) {
    ...
    try {
        const response = await fetch(`/api/tournaments/squads/delete/${equipoId}`, {
            method: 'GET'
        });
